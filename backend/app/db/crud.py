...
from pymongo.errors import DuplicateKeyError # Needed for unique constraints
import uuid
...
# --- Database Access ---
from .database import get_database # Assumes get_database() returns Optional[AsyncIOMotorDatabase]

# --- Pydantic Models ---
# Adjust paths if your models directory is different
...
# Ensure logging is configured elsewhere
...
async def transaction():
    ...
    db = get_database()
    # Use 'is None' check for database object
...
def with_transaction(func):
    """Decorator to wrap functions in a transaction if no session is provided."""
    @wraps(func)
    async def wrapper(*args, **kwargs):
        ...
        if session is not None:
             ...
             try:
                 ...
                 # Log error if transaction failed at this top level
                 logger.error(f"Transaction failed for function {func.__name__}: {e}")
                 # Decide on return value for transaction failure (None or re-raise)
                 return None # Or raise e
    return wrapper

def soft_delete_filter(include_deleted: bool = False) -> Dict[str, Any]:
    """Helper function to create filter for soft delete (assumes 'deleted_at' field)."""
...
def _get_collection(collection_name: str) -> Optional[AsyncIOMotorCollection]:
    ...
    db = get_database()
    # Corrected check: Use 'is not None' instead of 'if db:'
    if db is not None:
        return db[collection_name]
    # Log error if db connection failed (get_database returned None)
    logger.error("Database connection is not available (db object is None). Cannot get collection.")
    return None

# --- School CRUD Functions ---
# (Consistent pattern applied by user)
@with_transaction
async def create_school(school_in: SchoolCreate, session=None) -> Optional[School]:
    ...
    if collection is None: return None # Corrected check
    ...
    try:
        ...
        if created_doc: logger.info(f"Successfully created school: {new_school_id}"); return School(**created_doc)
...
async def get_school_by_id(school_id: uuid.UUID, include_deleted: bool = False, session=None) -> Optional[School]:
    ...
    if school_doc: return School(**school_doc)
...
async def get_all_schools(skip: int = 0, limit: int = 100, include_deleted: bool = False, session=None) -> List[School]:
    ...
    if collection is None: return schools_list # Corrected check
    ...
    try:
        ...
        async for doc in cursor:
            schools_list.append(School(**doc))
...
async def update_school(school_id: uuid.UUID, school_in: SchoolUpdate, session=None) -> Optional[School]:
    ...
    if updated_doc: return School(**updated_doc)
...
async def delete_school(school_id: uuid.UUID, hard_delete: bool = False, session=None) -> bool:
    collection = _get_collection(SCHOOL_COLLECTION); now = datetime.now(timezone.utc)
    if collection is None: return False # Corrected check
...
async def create_teacher(teacher_in: TeacherCreate, session=None) -> Optional[Teacher]:
    ...
    if created_doc: return Teacher(**created_doc)
...
async def get_teacher_by_id(user_id: uuid.UUID, include_deleted: bool = False, session=None) -> Optional[Teacher]:
    ...
    if teacher_doc: return Teacher(**teacher_doc)
...
async def get_all_teachers(skip: int = 0, limit: int = 100, include_deleted: bool = False, session=None) -> List[Teacher]:
    ...
    if collection is None: return teachers_list # Corrected check
    query = soft_delete_filter(include_deleted)
...
async def update_teacher(user_id: uuid.UUID, teacher_in: TeacherUpdate, session=None) -> Optional[Teacher]:
    ...
    update_data.pop("_id", None); update_data.pop("user_id", None); update_data.pop("created_at", None); update_data.pop("deleted_at", None)
    ...
    query_filter = {"_id": user_id, "deleted_at": None}
    try:
        updated_doc = await collection.find_one_and_update( query_filter, {"$set": update_data}, return_document=ReturnDocument.AFTER, session=session)
        if updated_doc: return Teacher(**updated_doc)
...
async def delete_teacher(user_id: uuid.UUID, hard_delete: bool = False, session=None) -> bool:
    ...
    count = 0
    try:
        if hard_delete: result = await collection.delete_one({"_id": user_id}, session=session); count = result.deleted_count
        else:
            now = datetime.now(timezone.utc)
            result = await collection.update_one({"_id": user_id, "deleted_at": None},{"$set": {"deleted_at": now, "updated_at": now}}, session=session); count = result.modified_count
...
async def create_class_group(class_group_in: ClassGroupCreate, session=None) -> Optional[ClassGroup]:
    ...
    if created_doc: return ClassGroup(**created_doc)
...
async def get_all_class_groups( teacher_id: Optional[uuid.UUID] = None, school_id: Optional[uuid.UUID] = None, skip: int = 0, limit: int = 100, include_deleted: bool = False, session=None) -> List[ClassGroup]:
    ...
    if collection is None: return items_list # Corrected check
    ...
    try:
        ...
        async for doc in cursor:
            items_list.append(ClassGroup(**doc))
...
async def update_class_group(class_group_id: uuid.UUID, class_group_in: ClassGroupUpdate, session=None) -> Optional[ClassGroup]:
    ...
    update_data.pop("_id", None); update_data.pop("created_at", None); update_data.pop("deleted_at", None)
    ...
    try:
        updated_doc = await collection.find_one_and_update( query_filter, {"$set": update_data}, return_document=ReturnDocument.AFTER, session=session)
        if updated_doc: return ClassGroup(**updated_doc)
...
async def delete_class_group(class_group_id: uuid.UUID, hard_delete: bool = False, session=None) -> bool:
    ...
    count = 0
    try:
        if hard_delete: result = await collection.delete_one({"_id": class_group_id}, session=session); count = result.deleted_count
        else:
             now = datetime.now(timezone.utc)
             result = await collection.update_one({"_id": class_group_id, "deleted_at": None},{"$set": {"deleted_at": now, "updated_at": now}}, session=session); count = result.modified_count
...
async def create_student(student_in: StudentCreate, session=None) -> Optional[Student]:
    ...
    student_doc = student_in.model_dump(); student_doc["_id"] = internal_id
    student_doc["created_at"] = now; student_doc["updated_at"] = now; student_doc["deleted_at"] = None
    ...
    try: inserted_result = await collection.insert_one(student_doc, session=session)
    except DuplicateKeyError: ext_id = student_doc.get('external_student_id'); logger.warning(f"Duplicate external_student_id: '{ext_id}'"); return None
    except Exception as e: logger.error(f"Error inserting student: {e}", exc_info=True); return None
    if inserted_result.acknowledged: created_doc = await collection.find_one({"_id": internal_id}, session=session)
    else: logger.error(f"Insert student not acknowledged: {internal_id}"); return None
    if created_doc: return Student(**created_doc)
    else: logger.error(f"Failed retrieve student post-insert: {internal_id}"); return None
...
async def get_student_by_id(student_internal_id: uuid.UUID, include_deleted: bool = False, session=None) -> Optional[Student]:
    ...
    query = {"_id": student_internal_id}; query.update(soft_delete_filter(include_deleted))
    try: student_doc = await collection.find_one(query, session=session)
    except Exception as e: logger.error(f"Error getting student: {e}", exc_info=True); return None
    if student_doc: return Student(**student_doc)
    else: logger.warning(f"Student {student_internal_id} not found."); return None
...
async def get_all_students( external_student_id: Optional[str] = None, first_name: Optional[str] = None, last_name: Optional[str] = None, year_group: Optional[str] = None, skip: int = 0, limit: int = 100, include_deleted: bool = False, session=None) -> List[Student]:
    ...
    if collection is None: return students_list # Corrected check
    filter_query = soft_delete_filter(include_deleted)
    ...
    except Exception as e: logger.error(f"Error getting all students: {e}", exc_info=True)
...
async def update_student(student_internal_id: uuid.UUID, student_in: StudentUpdate, session=None) -> Optional[Student]:
    ...
    if not update_data: logger.warning(f"No update data for student {student_internal_id}"); return await get_student_by_id(student_internal_id, include_deleted=False, session=session)
    update_data["updated_at"] = now; logger.info(f"Updating student {student_internal_id}")
    query_filter = {"_id": student_internal_id, "deleted_at": None}
    try:
        updated_doc = await collection.find_one_and_update( query_filter, {"$set": update_data}, return_document=ReturnDocument.AFTER, session=session)
        if updated_doc: return Student(**updated_doc)
        else: logger.warning(f"Student {student_internal_id} not found or already deleted for update."); return None
    except DuplicateKeyError: ext_id = update_data.get('external_student_id'); logger.warning(f"Duplicate external_student_id on update: '{ext_id}'"); return None
    except Exception as e: logger.error(f"Error during student update operation: {e}", exc_info=True); return None
...
async def delete_student(student_internal_id: uuid.UUID, hard_delete: bool = False, session=None) -> bool:
    ...
    count = 0
    try:
        if hard_delete: result = await collection.delete_one({"_id": student_internal_id}, session=session); count = result.deleted_count
        else:
             now = datetime.now(timezone.utc)
             result = await collection.update_one({"_id": student_internal_id, "deleted_at": None},{"$set": {"deleted_at": now, "updated_at": now}}, session=session); count = result.modified_count
...
async def get_all_assignments( class_group_id: Optional[uuid.UUID] = None, skip: int = 0, limit: int = 100, include_deleted: bool = False, session=None) -> List[Assignment]:
    ...
    if collection is None: return assignments_list # Corrected check
    filter_query = soft_delete_filter(include_deleted)
    ...
    try:
        cursor = collection.find(filter_query, session=session).skip(skip).limit(limit)
        async for doc in cursor:
            assignments_list.append(Assignment(**doc))
...
async def create_document(document_in: DocumentCreate, session=None) -> Optional[Document]:
    ...
    try:
        ...
        if created_doc: return Document(**created_doc)
...
async def get_document_by_id(document_id: uuid.UUID, include_deleted: bool = False, session=None) -> Optional[Document]:
    ...
    query = {"_id": document_id}; query.update(soft_delete_filter(include_deleted))
    try: doc = await collection.find_one(query, session=session)
...
async def update_document_status(document_id: uuid.UUID, status: DocumentStatus, session=None) -> Optional[Document]:
    ...
    query_filter = {"_id": document_id, "deleted_at": None}
    try:
        updated_doc = await collection.find_one_and_update(query_filter, {"$set": update_data}, return_document=ReturnDocument.AFTER, session=session)
...
async def delete_document(document_id: uuid.UUID, hard_delete: bool = False, session=None) -> bool:
    ...
    count = 0
    try:
        if hard_delete: result = await collection.delete_one({"_id": document_id}, session=session); count = result.deleted_count
        else:
             now = datetime.now(timezone.utc)
             result = await collection.update_one({"_id": document_id, "deleted_at": None},{"$set": {"deleted_at": now, "updated_at": now}}, session=session); count = result.modified_count
...
async def create_result(result_in: ResultCreate, session=None) -> Optional[Result]:
    ...
    try:
        ...
        if created_doc: return Result(**created_doc)
...
async def get_result_by_id(result_id: uuid.UUID, include_deleted: bool = False, session=None) -> Optional[Result]:
    ...
    query = {"_id": result_id}; query.update(soft_delete_filter(include_deleted))
    try: doc = await collection.find_one(query, session=session)
...
async def update_result(result_id: uuid.UUID, result_in: ResultUpdate, session=None) -> Optional[Result]:
    ...
    query_filter = {"_id": result_id, "deleted_at": None}
    try:
        updated_doc = await collection.find_one_and_update(query_filter, {"$set": update_data}, return_document=ReturnDocument.AFTER, session=session)
        if updated_doc: return Result(**updated_doc)
...
async def delete_result(result_id: uuid.UUID, hard_delete: bool = False, session=None) -> bool:
    ...
    count = 0
    try:
        if hard_delete: result = await collection.delete_one({"_id": result_id}, session=session); count = result.deleted_count
        else:
             now = datetime.now(timezone.utc)
             result = await collection.update_one({"_id": result_id, "deleted_at": None},{"$set": {"deleted_at": now, "updated_at": now}}, session=session); count = result.modified_count
...
async def bulk_create_schools(schools_in: List[SchoolCreate], session=None) -> List[School]:
    collection = _get_collection(SCHOOL_COLLECTION)
    if collection is None: return [] # Corrected check
    ...
    try:
        ...
        if result.acknowledged:
            ...
            if inserted_ids:
                ...
                async for doc in cursor: created_schools.append(School(**doc))
...
async def bulk_update_schools(updates: List[Dict[str, Any]], session=None) -> List[School]:
    ...
    try:
        for update_item in updates:
            school_id = update_item.get("id"); update_model_data = update_item.get("data") # Renamed variable
            # Ensure data is a dict before trying SchoolUpdate.model_validate
            ...
            try:
                ...
                update_model = SchoolUpdate.model_validate(update_model_data) # Use new variable name
            except Exception as validation_err:
                logger.warning(f"Skipping item due to validation error for school {school_id}: {validation_err}")
                continue
            ...
            result = await collection.find_one_and_update(
                {"_id": school_id, "deleted_at": None}, {"$set": update_doc},
                return_document=ReturnDocument.AFTER, session=session )
            if result: updated_schools.append(School(**result))
...
async def bulk_delete_schools(school_ids: List[uuid.UUID], hard_delete: bool = False, session=None) -> int:
    collection = _get_collection(SCHOOL_COLLECTION)
    if collection is None or not school_ids: return 0 # Corrected check
    deleted_count = 0
    try:
        if hard_delete: result = await collection.delete_many( {"_id": {"$in": school_ids}}, session=session); deleted_count = result.deleted_count
        else: result = await collection.update_many({"_id": {"$in": school_ids}, "deleted_at": None},{"$set": {"deleted_at": datetime.now(timezone.utc), "updated_at": datetime.now(timezone.utc)}},session=session); deleted_count = result.modified_count
...
async def validate_school_teacher_relationship( school_id: uuid.UUID, teacher_id: uuid.UUID, session=None) -> bool:
    ...
    return teacher and teacher.school_id == school_id
async def validate_class_group_relationships( class_group_id: uuid.UUID, teacher_id: uuid.UUID, school_id: uuid.UUID, session=None) -> bool:
    ...
    if class_group is None: return False # Corrected check
...
async def validate_student_class_group_relationship( student_id: uuid.UUID, class_group_id: uuid.UUID, session=None) -> bool:
    class_group = await get_class_group_by_id(class_group_id, include_deleted=False, session=session)
    # Corrected check: Ensure class_group is not None before accessing student_ids
    return class_group is not None and student_id in class_group.student_ids
...
async def get_schools_with_filters(
    ...
    sort_criteria = [(sort_by, sort_order)] if sort_by else None; schools = []
    try:
        cursor = collection.find(query, session=session)
        if sort_criteria: cursor = cursor.sort(sort_criteria)
        cursor = cursor.skip(skip).limit(limit)
        async for doc in cursor: schools.append(School(**doc))
...
async def get_teachers_by_school(
    ...
    try:
        cursor = collection.find(query, session=session).skip(skip).limit(limit)
        async for doc in cursor: teachers.append(Teacher(**doc))
...
# --- Final Placeholder ---
# (All core entities now have basic CRUD with consistent patter